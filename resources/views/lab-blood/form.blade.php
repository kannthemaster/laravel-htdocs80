...
<div class="form-group {{ $errors->has('report_by') ? 'has-error' : ''}}">
    ...
    <select class="form-control" name="report_by" id="report_by">
        ...
        @foreach ($users as $key => $value)
            <option value="{{ $key }}" @if (isset($labblood->report_by) && $key == $labblood->report_by) selected="selected" @endif>
                {{ $value }}</option>
        @endforeach
    </select>
...
<div class="form-group {{ $errors->has('approve_by') ? 'has-error' : ''}}">
    ...
    <select class="form-control" name="approve_by" id="approve_by">
        <option value=""></option>
        @foreach ($users as $key => $value)
            <option value="{{ $key }}" @if (isset($labblood->approve_by) && $key == $labblood->approve_by) selected="selected" @endif>
                {{ $value }}</option>
        @endforeach
    </select>
...
<div class="form-group {{ $errors->has('syphilis') ? 'has-error' : ''}}">
    ...
    <select class="form-control" name="syphilis" id="syphilis">
        @foreach (App\Models\LabBlood::$option as $key => $value)
